  ...
  const loop = makeLoop((dt, t) => {
    ...
    if (!over) {
      ...
      for (const o of obstacles) {
        const sx = o.x - worldX;
        if (!o.passed && sx < playerX && !playerAirborne) {
          // only fails if actually overlapping player x-range at ground level
        }
    ...
    const jumpP = jumping ? jumpT / jumpDur : 0;
    const jumpOffset = jumping ? jumpHeight * 4 * jumpP * (1 - jumpP) : 0;
    const py = groundY - playerR - jumpOffset;
    drawNicHead(g, {
      x: playerX,
      y: py,
      r: playerR,
      ...
      tooth: theme.ink,
      // hangtime tips him back, the landing snaps him level
      lean: -jumpOffset * 0.006,
      gape: jumping ? 0.35 : 0,
      scowl: over ? 1 : 0,
    });
